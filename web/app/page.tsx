...
import KsTable, { Row } from './components/KsTable';
...
export default async function Home({
  ...
  const isBeforeLineups = isViewingToday && parseInt(etHourStr, 10) < 11;
  ...
  try {
    const sql = getDb();
    rows = (await sql`
      SELECT * FROM ks_predictions
      WHERE game_date = ${validDate}::date
      ORDER BY pred_k DESC
    `) as Row[];
    ...
    <main style={{ minHeight: '100vh', background: 'var(--ev-bg)', padding: '32px 20px 60px' }}>
      <div style={{ maxWidth: '1480px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: '20px', marginTop: '8px', flexWrap: 'wrap' }}>
          ...
          <span style={{ ...LABEL, color: 'var(--ev-gold)' }}>MY LINE = CUSTOM K TOTAL</span>
        </div>
