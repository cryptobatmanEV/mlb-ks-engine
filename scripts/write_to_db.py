...
INSERT INTO ks_predictions (
    ...
    ud_line, ud_is_alt, ud_side, model_prob_ud_line, edge_ud,
    book_markets,
    ...
    %(ud_line)s, %(ud_is_alt)s, %(ud_side)s, %(model_prob_ud_line)s, %(edge_ud)s,
    %(book_markets)s,
...
ON CONFLICT (game_date, pitcher, game_pk) DO UPDATE SET
    ...
    has_line             = CASE WHEN EXCLUDED.has_line IS FALSE AND ks_predictions.has_line IS TRUE THEN ks_predictions.has_line             ELSE EXCLUDED.has_line             END,
    book_line            = CASE WHEN EXCLUDED.has_line IS FALSE AND ks_predictions.has_line IS TRUE THEN ks_predictions.book_line            ELSE EXCLUDED.book_line            END,
    book_side            = CASE WHEN EXCLUDED.has_line IS FALSE AND ks_predictions.has_line IS TRUE THEN ks_predictions.book_side            ELSE EXCLUDED.book_side            END,
    best_book            = CASE WHEN EXCLUDED.has_line IS FALSE AND ks_predictions.has_line IS TRUE THEN ks_predictions.best_book            ELSE EXCLUDED.best_book            END,
    best_odds            = CASE WHEN EXCLUDED.has_line IS FALSE AND ks_predictions.has_line IS TRUE THEN ks_predictions.best_odds            ELSE EXCLUDED.best_odds            END,
    book_implied         = CASE WHEN EXCLUDED.has_line IS FALSE AND ks_predictions.has_line IS TRUE THEN ks_predictions.book_implied         ELSE EXCLUDED.book_implied         END,
    model_prob_book_line = CASE WHEN EXCLUDED.has_line IS FALSE AND ks_predictions.has_line IS TRUE THEN ks_predictions.model_prob_book_line ELSE EXCLUDED.model_prob_book_line END,
    edge_book            = CASE WHEN EXCLUDED.has_line IS FALSE AND ks_predictions.has_line IS TRUE THEN ks_predictions.edge_book            ELSE EXCLUDED.edge_book            END,
    adj_k                = CASE WHEN EXCLUDED.has_line IS FALSE AND ks_predictions.has_line IS TRUE THEN ks_predictions.adj_k                ELSE EXCLUDED.adj_k                END,
    pp_line              = CASE WHEN EXCLUDED.pp_line IS NULL AND ks_predictions.pp_line IS NOT NULL THEN ks_predictions.pp_line              ELSE EXCLUDED.pp_line              END,
    pp_side              = CASE WHEN EXCLUDED.pp_line IS NULL AND ks_predictions.pp_line IS NOT NULL THEN ks_predictions.pp_side              ELSE EXCLUDED.pp_side              END,
    model_prob_pp_line   = CASE WHEN EXCLUDED.pp_line IS NULL AND ks_predictions.pp_line IS NOT NULL THEN ks_predictions.model_prob_pp_line   ELSE EXCLUDED.model_prob_pp_line   END,
    edge_pp              = CASE WHEN EXCLUDED.pp_line IS NULL AND ks_predictions.pp_line IS NOT NULL THEN ks_predictions.edge_pp              ELSE EXCLUDED.edge_pp              END,
    ud_line              = CASE WHEN EXCLUDED.ud_line IS NULL AND ks_predictions.ud_line IS NOT NULL THEN ks_predictions.ud_line              ELSE EXCLUDED.ud_line              END,
    ud_is_alt            = CASE WHEN EXCLUDED.ud_line IS NULL AND ks_predictions.ud_line IS NOT NULL THEN ks_predictions.ud_is_alt            ELSE EXCLUDED.ud_is_alt            END,
    ud_side              = CASE WHEN EXCLUDED.ud_line IS NULL AND ks_predictions.ud_line IS NOT NULL THEN ks_predictions.ud_side              ELSE EXCLUDED.ud_side              END,
    model_prob_ud_line   = CASE WHEN EXCLUDED.ud_line IS NULL AND ks_predictions.ud_line IS NOT NULL THEN ks_predictions.model_prob_ud_line   ELSE EXCLUDED.model_prob_ud_line   END,
    edge_ud              = CASE WHEN EXCLUDED.ud_line IS NULL AND ks_predictions.ud_line IS NOT NULL THEN ks_predictions.edge_ud              ELSE EXCLUDED.edge_ud              END,
    book_markets         = CASE WHEN EXCLUDED.has_line IS FALSE AND ks_predictions.has_line IS TRUE THEN ks_predictions.book_markets         ELSE EXCLUDED.book_markets         END,
            ...
            with conn.cursor() as cur:
                ...
                for _, row in df.iterrows():
                    ...
                    cur.execute(UPSERT, {
                        ...
                        'ud_line':   _clean(row.get('ud_line')),
                        'ud_is_alt': _bool(row.get('ud_is_alt')),
                        'ud_side':   _str(row.get('ud_side')),
                        'model_prob_ud_line': _clean(row.get('model_prob_ud_line')),
                        'edge_ud': _clean(row.get('edge_ud')),
