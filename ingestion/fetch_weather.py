...
Adapted from mlb-prop-engine's ingestion/fetch_weather.py (same stadium
metadata and math) -- this copy reads from statcast_pitching.parquet instead
of the batted-ball store.
"""
import os
import sys
import time
...
def calc_wind_favor(wind_speed, wind_dir, cf_bearing):
    """Component of wind blowing toward the outfield, in mph (vectorized)."""
...
def fetch_historical(game_team_df):
    ...
    Makes ~30 API calls total (one per team).
    """
...
if __name__ == '__main__':
    STORE = 'data/raw/statcast_pitching.parquet'
    OUT   = 'data/processed/weather.parquet'
